...
export function setupSocketAPI(http) {
    ...
    gIo.on('connection', socket => {
        ...
        socket.on('order-set-status', status => {
            if (socket.myStatus === status) return
            if (socket.myStatus) {
                socket.leave(socket.myStatus)
                loggerService.info(`Socket is leaving topic ${socket.myStatus} [id: ${socket.id}]`)
            }
            socket.join(status)
            socket.myStatus = status
        })
